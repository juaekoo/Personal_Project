$(document).ready(function(){
    $('#submit').click(function(){
        ...
        const emailFormat = RegExp(/^[A-Za-z0-9_\-\'\.]+.@[A-Za-z0-9]+\.[A-Za-z0-9]+$/);
        const passwordFormat = RegExp(/^[A-Za-z0-9]{8,}$/);

        var email = $('input[name="email"]').val();
        var password = $('input[name="password"]').val();
        if(!email) {
            $('input[name="email"]').css('border', 'solid 1px red');
            $('input[name="email"]').css('box-shado', '0px 0px 5px 3px red');
        } else if(!getCheck.emailFormat(email)){
            $('#alarm').html('이메일 형식이 올바르지 않습니다.')
        }
        if(!password) {
            $('input[name="password"]').css('border', 'solid 1px red');
            $('input[name="password"]').css('box-shado', '0px 0px 5px 3px red');
        } else if(!getCheck.passwordFormat(password)){
            $('#alarm').html('비밀번호 형식이 올바르지 않습니다.')
        }

        if(getCheck.emailFormat(email) && getCheck.passwordFormat(password)) {
